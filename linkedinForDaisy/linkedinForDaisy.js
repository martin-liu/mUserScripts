...
(function(){
  ...
  class LinkedinConnect {
    ...
    prepareButton() {
      var button = createElement('<button class="daisying-btn artdeco-button artdeco-button--3 artdeco-button--secondary" style="position:fixed;right:290px;top:62px;z-index:99999">666</button>');
      var autoButton = createElement('<button class="daisying-btn daisying-auto-btn artdeco-button artdeco-button--3 artdeco-button--secondary" style="position:fixed;right:150px;top:62px;z-index:99999">Auto 666</button>');
      const trigger = async () => {
      ...
      if (noteButton) {
        ...
        if (textArea.length) {
          ...
          if (inviteButton) {
            ...
            inviteButton.click();
            await wait(300);
            let gotItButton = [].find.call($(`${modalCls} button`), d => d.innerText=='Got it');
            if (gotItButton) {
              gotItButton.click();
            }
          }
  ...
  let Daisying = new LinkedinConnect({
    name:'Daisy',
    inviteNote: `Hi there, I am working directly with the Talent Acquisition team for Varis, a well-funded SaaS startup. Your background looks impressive and I’d like to connect with you for future job opportunities. Thank you!`
  });
  Daisying.execute();
